...
void printIntro();
void createFile();
void loadFile(int* memoryPtr, int* insRegPtr, int* insCPtr);
...
int main(void) {
	...
	createFile();
	loadFile(memoryPtr, insRegPtr, insCPtr);
...
void createFile() {
	FILE* fPtr;
	if ((fPtr = fopen("instruction.dat", "w")) == NULL) {
		puts("Error opening instruction.dat for w");
		return;
	}
...
void loadFile(int* memoryPtr, int* insRegPtr, int* insCPtr) {
	FILE* fPtr;
	if ((fPtr = fopen("instruction.dat", "r")) == NULL) {
		puts("Error opening instruction.dat for r");
		return;
	}
