...
void blank100() {
	...
	if ((wfPtr = fopen("nameage.dat", "wb")) == NULL) {
		puts("Error opening nameage.dat for wb");
		return;
	}
...
void printTen() {
	...
	if ((rfPtr = fopen("nameage.dat", "rb")) == NULL) {
		puts("Error opening nameage.dat for rb");
		return;
	}
...
void updateRecord() {
	...
	Person newData = { "", "", "0" };
	printf("%s", "Update existing lastName, firstName, newAge: ");
	char line[50];
	fgets(line, 50, stdin);
...
void deleteRecord() {
	...
	Person userEntry = { "", "", "0" };
	printf("%s", "Delete existing lastName, firstName, age: ");
	char line[50];
	fgets(line, 50, stdin);
	char* token = strtok(line, ", \n");
